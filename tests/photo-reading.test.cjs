...
(async () => {
  ...
  attempts=[];
  route.fetch=async(url,init)=>{attempts.push({url:String(url),method:init?.method||'GET'});return new Response('/9j/IMAGE');};
  const imageRequest=()=>new Request('https://app.test/api/gas',{method:'POST',body:JSON.stringify({action:'getMapBase64',id:'test&photo'})});
  response=await route.exports.POST(imageRequest());
  assert.equal((await response.json()).base64,'/9j/IMAGE');assert.equal(attempts.length,1);assert.equal(attempts[0].method,'GET');
  assert.equal(new URL(attempts[0].url).searchParams.get('action'),'getMapBase64');assert.equal(new URL(attempts[0].url).searchParams.get('id'),'test&photo');
  attempts=[];
  route.fetch=async(url,init)=>{attempts.push({url:String(url),method:init?.method||'GET'});return attempts.length===1?json({success:false,error:'unknown action'}):new Response('/9j/LEGACY');};
  response=await route.exports.POST(imageRequest());assert.equal((await response.json()).base64,'/9j/LEGACY');assert.deepEqual(attempts.map(x=>x.method),['GET','POST']);
  ...
  let requestedId;
  gas.handleGetMapBase64=id=>{requestedId=id;return {success:true,base64:'/9j/GET'};};
  assert.equal(gas.doGet({parameter:{action:'getMapBase64',id:'photo-id'}}).success,true);assert.equal(requestedId,'photo-id');
  gas.handleGetMapBase64=()=>{throw new Error('permission denied');};
  assert.equal(gas.doGet({parameter:{action:'getMapBase64',id:'photo-id'}}).success,false);
  console.log('PASS: image GET route and legacy POST fallback, GAS GET handler and error response.');
